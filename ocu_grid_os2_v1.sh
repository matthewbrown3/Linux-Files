...
#####  CREATED BY MRB ON 7/25/2019  #####

#####  CONVERTED ALL ECHO FILES TO HEREDOCUMENTS  #####
#####  ADDED A PING CHECK TO THE CONTROLLING TERMINAL BEFORE CHECKING FOR OCU PORT NUMBER  #####
...
EOS
  ...
  DT_OCU_TERM="term0$LEADING_ZERO";
...
for OCU in `(grep 'host ocu' /etc/dhcpd.conf | awk '{print$2}'; grep 'mb' /home/termload/term0$LEADING_ZERO/printcap | awk '{print$2}') | sort -u`; do
...
printf "|%-11s |%-11s |%-16s |%-11s |%-11s |%-11s |%-11s |%-11s |%-11s |\n" $OCU $ONLINE $IP $DTFC $TYPE $VERSION $DT_OCU_TERM $OCU_PORT $TERMLOAD_PORT | sed 's/_/ /g';
echo "---------------------------------------------------------------------------------------------------------------------------";
done;
