...
##convert echo files to here docs
##convert the daily pass to hex
##convert the sicom pass to hex
##make all checks into functions
##change straight if statements to case statements
##change the dtd_json to just the last 4 of the md5sum string
...
echo "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
printf "|%-11s |%-11s |%-11s |%-11s |%-11s |%-12s |%-11s |%-13s |%-13s |%-11s |%-11s |%-11s |%-7s |%-11s |%-12s |%-13s |\n" Ktchn\ Scrn Name Config Type Ctrl\ Term Hardware Version KSD\ Status Routing\ Opt Bump\ Sever Sort\ Mode Beep Total? ORS DTD\ JSON.DAT Last\ Update;
echo "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
...
printf "|%-11s |%-11s |%-11s |%-11s |%-11s |%-12s |%-11s |%-13s |%-13s |%-11s |%-11s |%-11s |%-7s |%-11s |%-12s |%-13s |\n" $KS $NAME $CONFIG_TYPE $SCREEN_TYPE $CTRL_TERM $HARDWARE $VERSION $KSD $ROUTING_OPTION $BUMPSERVER $SORT_MODE $BEEP $TOTAL $ORS $DTD_JSON $DTD_JSON_TIME | sed 's/-/ /g' | sed 's/_/ /g';
echo "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
